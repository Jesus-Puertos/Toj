...
const MOCK_OBLIGACIONES: Obligacion[] = [
  {
    id: "ob-1",
    tipo_tramite: "Predial 2026 - Zongolica",
    monto_total: 2500,
  ...
  {
    id: "ob-3",
    tipo_tramite: "Licencia Municipal",
    monto_total: 1200,
...
export default async function CiudadanoDashboardPage() {
  ...
  // Foto de perfil: Google OAuth la guarda en avatar_url
  ...
  if (user) {
    // 1. ciudadano_id desde usuarios_plataforma
    ...
    if (ciudadanoId) {
      ...
      const { data } = await supabase
        .from("obligaciones")
        .select(
          "id, tipo_tramite, monto_total, fecha_vencimiento, estado_cumplimiento",
        )
  ...
  if (obligaciones.length === 0) obligaciones = MOCK_OBLIGACIONES;

  // Mapeo visual del estado KYC
  ...
  > = {
    Verificado: {
      ...
      icon: "verified",
      color: "text-primary bg-primary-fixed/40",
    },
    EnProceso: {
      label: "KYC en Revisión",
      icon: "pending",
    ...
    Pendiente: {
      ...
      color: "text-toj-text-secondary bg-surface-container",
    },
  };
    ...
    <>
      <main className="px-5 pt-5 space-y-6">
        ...
        <header className="sticky top-0 bg-surface z-10 flex items-center justify-between py-4 border-b border-outline-variant -mx-5 px-5">
          <span className="text-primary font-bold text-[18px] tracking-tight">
            TOJ Platform
          </span>
          <div className="flex items-center gap-3">
            ...
            {/* Avatar → link a perfil (foto real o iniciales) */}
            <Link
            ...
            >
              <Avatar
                ...
                size="md"
                className="ring-2 ring-primary/30 hover:ring-primary transition-all"
              />
              <span
                className={`absolute -top-1 -right-1 border-2 border-surface rounded-full px-1 py-0.5 text-[8px] font-bold leading-none ${estadoKyc === "Verificado" ? "bg-primary text-on-primary" : "bg-toj-terracotta text-white"}`}
              >
        ...
        <section className="space-y-2">
          ...
          <Link
            href="/kyc"
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full w-fit transition-opacity hover:opacity-80 ${currentKyc.color}`}
          >
            <span
            ...
            {obligaciones.length > 0 ? (
              ...
              <div className="p-8 text-center bg-surface-container rounded-3xl border border-dashed border-outline-variant">
                <p className="text-on-surface-variant text-body-sm italic">
                  No tienes obligaciones pendientes por ahora.
                </p>
              </div>
            )}
          </div>
        ...
        <section className="space-y-3 pb-4">
          <div className="flex items-center justify-between">
            ...
            <button className="text-body-sm font-semibold text-primary">
              Gestionar
            </button>
          </div>
          <CarpetaDigital />
